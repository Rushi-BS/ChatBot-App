...
const ProfileForm: React.FC = () => {
  const [profileData, setProfileData] = useState({
    ...
    phoneNo: "",
    location: "",
    profilePhoto: null as File | null, // Change the type to File | null,
  });
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      setProfileData((prev) => ({ ...prev, profilePhoto: files[0] as File }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response: ApiResponse<any> = await apiHelper.post(
        "/profile",
        formData
      );
      ...
      if (response.data.error) {
        ...
        // You can perform additional actions after successful profile update
      }
    } catch (error) {
